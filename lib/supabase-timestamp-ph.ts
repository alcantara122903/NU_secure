/**
 * Timestamps sent to Supabase/Postgres as ISO-8601 with explicit Philippines
 * offset (UTC+8, no DST). Same absolute instant as UTC; wire format uses PHT.
 *
 * Edge: keep `supabase/functions/office-exit-scan/manila-timestamp.ts` in sync.
 */
...
/**
 * Format `date` for `timestamptz` columns: civil time in Manila + `+08:00`.
 */
